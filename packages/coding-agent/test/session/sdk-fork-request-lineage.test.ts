...
import { type ForkRequestSnapshot, recordForkRequestSnapshot } from "@oh-my-pi/pi-coding-agent/session/fork-context";
	...
	globalThis.fetch = Object.assign(
		async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
			...
			captured.push({
				...
				body: JSON.parse(new TextDecoder().decode(plain)) as Record<string, unknown>,
			});
...
describe("sdk fork request lineage", () => {
	it("never self-forks a root session and keeps parent lineage in a forked child until /clear", async () => {
		...
		try {
			...
			const parentSessionHeader = header(require(firstRequest, "first parent request"), "session-id");
			const parentThreadHeader = header(require(firstRequest, "first parent request"), "thread-id");
			...
			expect(header(require(parentTurn2, "parent turn 2"), "x-codex-parent-thread-id")).toBeUndefined();
			expect(header(require(parentTurn2, "parent turn 2"), "session-id")).toBe(parentSessionHeader);
			// Turn 2 must not replay turn 1's serialized input ahead of its own:
			// a self-fork duplicates the leading turn verbatim.
			expect(bodyContains(require(parentTurn2, "parent turn 2"), "MARKER-ALPHA")).toBe(1);
			...
			const spawnResult = await require(taskTool, "task tool").execute("tc-fork-spawn", {
				agent: "task",
				name: "ForkedChild",
				task: "probe the fork",
				fork: "all",
			...
			recordForkRequestSnapshot(childManager, require(forkRequest, "fork request"));
			...
			expect(header(require(childTurn1, "child turn 1"), "session-id")).toBe(parentSessionHeader);
			expect(header(require(childTurn1, "child turn 1"), "x-codex-parent-thread-id")).toBe(parentThreadHeader);
			expect(header(require(childTurn2, "child turn 2"), "session-id")).toBe(parentSessionHeader);
			expect(header(require(childTurn2, "child turn 2"), "x-codex-parent-thread-id")).toBe(parentThreadHeader);
			// Child keeps its own distinct thread identity.
			const childThread = header(require(childTurn1, "child turn 1"), "thread-id");
			expect(childThread).toBeDefined();
			expect(childThread).not.toBe(parentThreadHeader);
			expect(header(require(childTurn2, "child turn 2"), "thread-id")).toBe(childThread);
			// The parent's wire prefix is replayed once — never re-duplicated by
			// a self-reference on the child's second turn.
			expect(bodyContains(require(childTurn1, "child turn 1"), "MARKER-ALPHA")).toBe(1);
			expect(bodyContains(require(childTurn2, "child turn 2"), "MARKER-ALPHA")).toBe(1);
			...
			expect(header(require(afterClear, "post-clear request"), "session-id")).not.toBe(parentSessionHeader);
			expect(header(require(afterClear, "post-clear request"), "x-codex-parent-thread-id")).toBeUndefined();
			expect(bodyContains(require(afterClear, "post-clear request"), "MARKER-ALPHA")).toBe(0);
		} finally {
