...
- Terminate processes you started deliberately: identify the exact process (PID, pidfile, or application shutdown command) rather than broad pattern kills that can take down unrelated work. Other agents share this machine.
- If a handoff goes wrong repeatedly, step back before the next attempt: name the plausible causes, rank them, and work the most likely first instead of retrying the same command. When the obstacle is the plan itself rather than its execution (the spec points at the wrong layer, or the approach cannot satisfy the constraints), say so with evidence instead of forcing a workaround.
