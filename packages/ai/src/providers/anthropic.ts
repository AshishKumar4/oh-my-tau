...
function applyHeadCaching(
	...
	// The captured Claude Code client anchors its last system block as well as
	// its identity block, so the profile adds the second breakpoint the shared
	// path deliberately skips.
	if (harnessProfile === "claude-code" && systemBlocks && systemBlocks.length > 0) {
		const lastBlock = systemBlocks[systemBlocks.length - 1];
