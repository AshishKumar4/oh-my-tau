...
export class RemoteAuthCredentialStore implements AuthCredentialStore {
	...
	#deleteSnapshotBlocks(credentialId: number): void {
		...
		// Mirror the store contract: a manual hold is not a rate-limit block and survives.
		const held = entry.blocks.filter(block => block.blockScope === CREDENTIAL_HOLD_SCOPE);
		if (held.length === entry.blocks.length) return;
		const next: SnapshotEntry = { ...entry };
		if (held.length > 0) next.blocks = held;
